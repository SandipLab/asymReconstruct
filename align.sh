# Sample script for the sorting and manual alignment of asymmetric particles in AROMA pipeline
#
#
# Usage: bash [script.sh] [input.star] [classes] [Ns] [composition] [output.star]
#
...
# composition         An Ns-letter string standing for the target particle composition of subunit conformations. This script is designated for sorting and aligning cyclically organized oligomeric single particles with its symmetry-applied reconstruction performed in RELION software. The letters used in this string should conform to those defined by the above "classes" argument. 
...
# Sort and Align
awk -v class_assignments=$2 -v Ns=$3 -v composition=$4 -v col_Micrograph=$(col_id _rlnMicrographName $1) -v col_X=$(col_id _rlnCoordinateX $1) -v col_Y=$(col_id _rlnCoordinateY $1) -v col_Rot=$(col_id _rlnAngleRot $1) -v col_Class=$(col_id _rlnClassNumber $1) -v Ncol=$(number_cols $1) '
...
END{
	for (ptl in ptl_list){
		if(is_circularly_equal(as_string(ptl_composition, ptl, Ns), composition)!=-1){
			print subunits[ptl " " ptl_ang_at_rank[ptl " " is_circularly_equal(as_string(ptl_composition, ptl,Ns), composition)]]
		}
	}
}' $1 > $5
